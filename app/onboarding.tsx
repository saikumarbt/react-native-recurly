...
import "@/global.css";
import { getMonthlyEquivalent, resolveNextRenewal } from "@/lib/billing";
...
const Onboarding = () => {
  ...
  const monthlyTotal = selectedBrands.reduce(
    (sum, b) => sum + getMonthlyEquivalent(priceFor(b.title, b.price), "monthly"),
    0,
  );
  ...
  const addSelected = () => {
    ...
    for (const brand of selectedBrands) {
      ...
      addSubscription({
        ...
        startDate: now,
        renewalDate:
          resolveNextRenewal(now, "monthly")?.toISOString() ?? undefined,
      });
      count += 1;
    }
    finish(count);
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      ...
      {step === "confirm" && (
        ...
        >
          <Text className="auth-title mt-4">Confirm what you pay</Text>
          <Text className="auth-subtitle text-left">
            Adjust the defaults to what you actually pay in {baseCurrency}. You
            can fine-tune any of these later.
          </Text>
          ...
          {selectedBrands.map((brand) => (
            ...
            >
              ...
              <TextInput
                className="w-28 rounded-xl border border-border bg-background px-3 py-2 text-right text-base font-sans-bold text-primary"
