...
- **Single app-wide base currency, no FX** (not per-sub multi-currency / conversion / dual-display). Users enter amounts in their own currency; changeable in Settings. No `RatesProvider`/FX.
...
| Data leak across accounts on one device                        | Out of scope — single-user-per-device assumption; portability is the Pro backup/sync story, not local multi-account                                                                                                          |
